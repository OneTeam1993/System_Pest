        ...
        public AssetInfo GetAssetByTag(AssetInfo item)
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(mConnStr))
                {
                    ...
                    #region GET THE LINKED DRIVER
                    try
                    {
                        currDriver = driverRepo.Get(item.DriverID);
                        if (item == null) currAsset.Name = "Driver Not Found";
                        //Logger.LogEvent("Check Asset:" + currAsset.Name, System.Diagnostics.EventLogEntryType.Error);
                        //Logger.LogEvent("GET JobUser: " + currDriver.Name, System.Diagnostics.EventLogEntryType.Warning);

                    }
                    catch (Exception ex)
                    {
                        Logger.LogEvent("GET THE LINKED DRIVER: " + ex.Message, System.Diagnostics.EventLogEntryType.Error);
                    }
                    #endregion  
                    ...
                    try
                    {
                        using (MySqlCommand cmd = new MySqlCommand())
                        {
                            conn.Open();
                            cmd.Connection = conn;
                            cmd.CommandText = "UPDATE jobs SET asset_id = @AssetID WHERE timestamp between @StartTS and @EndTS and driver_name = @JobUser and job_status = @JobStatus";
                            cmd.Prepare();
                            cmd.Parameters.AddWithValue("@StartTS", DateTime.Today);
                            cmd.Parameters.AddWithValue("@EndTS", DateTime.Today.AddDays(1));
                            cmd.Parameters.AddWithValue("@JobUser", currDriver.Name);
                            cmd.Parameters.AddWithValue("@JobStatus", "Scheduled");
                            cmd.Parameters.AddWithValue("@AssetID", currAsset.AssetID);
                            cmd.ExecuteNonQuery();
                            conn.Close();
                        }
