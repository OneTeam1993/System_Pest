...
//Delete Row
var $tableItem = $('#pestsTreatment'), $removeItem = $('#deleteItem'), $editItem = $('#editItem');
...
$(document).ready(function () {
    ...
    $('#deleteItem').on('click', function () {
        ...
        var ids = $.map($tableItem.bootstrapTable('getSelections'), function (row) {
            ...
            // var item = JSON.stringify({ 'Commodity': row.Commodity });
...
$(function () {
    ...
    if (getSessionstorageValueRoleID == 1) {
        WebAPIDriver = 'http://117.120.7.120/spwebapi/api/driverinfo?ResellerID=' + '1' + '&CompanyID=' + '1';
    }

    else if (getSessionstorageValueRoleID == 2) {
        WebAPIDriver = 'http://117.120.7.120/spwebapi/api/driverinfo?ResellerID=' + getSessionstorageValueUserResellerID + '&CompanyID=' + getSessionstorageValueCompanyID;
    ...
    $.getJSON(WebAPIDriver, function (data) {
        $.each(data, function (index, item) {
            ...
            if (item.AssetID > 0) {
                $('#jobTechnician').append($('<option></option>').val(item.DriverID).html(item.Name));
...
$(function () {
    ...
    //Populate phone user
    var WebAPIPest = "http://track-asia.com/spwebapi/api/pestinfo";
...
$(function () {
    ...
    //Populate phone user
    var WebAPIPest = "http://track-asia.com/spwebapi/api/treatmentinfo";
...
$(function () {
    ...
    $('.SelectCompanyFilter').on('change', function () {
        ...
        if (selectedCompany != 0) {
            ...
            $.getJSON('http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + '&ResellerID=' + getReseller + '&CompanyID=' + selectedCompany, function (data) {
...
function setMarkers(map, assets, callback) {
    ...
    if (getSessionstorageValueRoleID == 1) {
        ...
        MarkerAPI = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + '&ResellerID=' + '1' + '&CompanyID=' + '1';

    } else if (getSessionstorageValueRoleID == 2) {

        MarkerAPI = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + getSessionstorageValueUserID + '&ResellerID=' + getSessionstorageValueUserResellerID + '&CompanyID=' + getSessionstorageValueCompanyID;
...
function setMarkersFilter(map, assets, callback) {
    ...
    if (getSessionstorageValueRoleID == 1) {
        ...
        if (selectedReseller != "") {
            url = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + '&ResellerID=' + selectedReseller + '&CompanyID=' + selectedCompany;
        } else {
            url = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + '&ResellerID=' + getReseller + '&CompanyID=' + selectedCompany;
        }

    } else if (getSessionstorageValueRoleID == 2) {

        url = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + getSessionstorageValueUserID + '&ResellerID=' + getSessionstorageValueUserResellerID + '&CompanyID=' + selectedCompany;

    } else if (getSessionstorageValueRoleID >= 3) {

        url = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + getSessionstorageValueUserID + '&ResellerID=' + getSessionstorageValueUserResellerID + '&CompanyID=' + getSessionstorageValueCompanyID;
    ...
    for (var i = 0; i < assets.length; i++) {
        ...
        if (assets[i].LastPos == null | assets[i].LastPos.Engine == null) {
            ...
            if (markers[i] && markers[i].setPosition) {
                if ($('#jobDestination').val() == 0) $("#side_bar").empty();
		...
		var getAddress = $('#jobDestination').val();
...
function editjob(id, job_id, flag) {
	...
	if (flag == 0 && getSessionstorageValueRoleID > 3){
	    alert('Completed Job cannot edit');
	} else {
	    bootbox.confirm("<div style='color:black'>" + 'Are you sure you want to edit this job?' + "</div>", function (result) {
	        if (result) {
	            ...
	            var WebAPI = 'http://117.120.7.120/spwebapi/api/jobinfo/' + job_id;
	            ...
	            $.ajax({
	                ...
	                success: function (res) {
	                    ...
	                    var timestamp = moment.utc(res.Timestamp).add('hours', 8).format('DD-MMM-YYYY, HH:mm');
	                    $('#reference').val(res.JobNumber);
	                    $('#jobid').val(res.JobID);
	                    $('#companyName').val(res.Company);
	                    $('#jobDestination').val(res.Destination);
	                    $('#jobPostal').val(res.Postal);
	                    $('#jobUnit').val(res.Unit);
	                    $('#jobPIC').val(res.PIC);
	                    $('#jobContactNumber').val(res.Phone);
	                    $('#jobTime').val(timestamp);
	                    $('#jobCharges').val(res.Amount);
	                    $('#customerEmail').val(res.CusEmail);
	                    $('#jobSite').val(res.Site);
	                    $('#jobRemarks').val(res.Remarks);
	                    $("#jobTechnician").val(res.DriverID);
	                    $("#jobReferenceNo").val(res.ReferenceNo);

	                    $('.selectpicker').selectpicker('refresh');

	                    var PestInfo = res.Pest;

	                    var rows = [];
	                    for (var i = 0; i < PestInfo.length; ++i) {

	                        rows.push({
	                            Pests: PestInfo[i].PestDesc,
	                            Treatments: PestInfo[i].TreatmentDesc,
	                            Index: PestInfo[i].ItemNo
	                        });
	                    }
	                    //alert(res[0].FlagValue);
...
function deletejob(id, job_id, flag) {
    ...
    if (flag == 0 && getSessionstorageValueRoleID > 3) {
        alert('Completed Job cannot delete');
    }
    else {
        bootbox.confirm("<div style='color:black'>" + 'Are you sure you want to delete this job?' + "</div>", function (result) {
            if (result) {
                //window.location.reload(true);
                $.niftyNoty({
                    type: 'success',
                ...
                var jobs = JSON.stringify({ 'JobID': job_id });
                var WebAPI = 'http://117.120.7.120/spwebapi/api/jobinfo/' + job_id;
...
function calculateDistances(pt, closest, numberOfResults) {
    ...
    $.ajax({
        url: 'http://117.120.7.120/dmswebapi/api/distancematrix?Origin=' + originPT[1] + '&Destination=' + finalClosest,
        type: 'GET',
        cache: false,
        dataType: 'json',
        success: function (json) {
            ...
            $.each(json, function (index, element) {
                if (element.status != 'OK') {
                    console.log(titleError + element.status);
                } else {
...
function searchDriver(id) {
    ...
    var apiDriver = 'http://117.120.7.120/spwebapi/api/driverinfo?ResellerID=' + getReseller + '&CompanyID=' + getCompany + '&AssetID=' + id;
    var apiDevice = 'http://117.120.7.120/spwebapi/api/deviceinfo?ResellerID=' + getReseller + '&CompanyID=' + getCompany + '&AssetID=' + id;
    //alert(apiDriver);
...
function searchJob() {
    ...
    var apiJobNumber = 'http://117.120.7.120/spwebapi/api/searchjob?JobNumber=' + getJobNumber;
    //alert(apiJobNumber);
        ...
        if (data.length == 1) {
            ...
            $.each(data, function (index, item) {
                ...
                if (item.JobStatus == "Completed") {
					...
					//console.log(item.DriverInfo.DriverID)
					var jobID = $('#jobid').val(item.JobID);
					var jobCompanyName = $('#companyName').val(item.Company);
					var jobAddress = $('#jobDestination').val(item.Destination);
					var jobPostal = $('#jobPostal').val(item.Postal);
					var jobUnitNo = $('#jobUnit').val(item.Unit);
					var jobPersonInCharge = $('#jobPIC').val(item.PIC);
					var jobContactNumber = $('#jobContactNumber').val(item.Phone);
					var jobScheduledTime = $('#jobTime').val(item.Timestamp);
					var jobCharges = $('#jobCharges').val(item.Amount);
					var jobCustomerEmail = $('#customerEmail').val(item.CusEmail);
					var jobSite = $('#jobSite').val(item.Site);
					var jobTechnician = $('#jobTechnician').val(item.DriverInfo.DriverID);
					var jobRemarks = $('#jobRemarks').val(item.Remarks);
...
function clearJobForms() {
	...
	$('#jobContractStart').val(moment().format('D-MMM-YYYY, HH:mm'));
	$('#jobContractEnd').val(moment().format('D-MMM-YYYY, HH:mm'));
...
function timeFunction(value) {
    ...
    var labelColor;
    var dateTimestamp = moment(value).add('hours', 8).format('DD-MMM-YYYY');
    var timestamp = moment(value).add('hours', 8).format('DD-MMM-YYYY HH:mm:ss A');
    ...
    return '<div class="label label-table label-' + labelColor + '">' + timestamp + '</div>';
...
function reloadJobsCreated()
    {
        ...
        var WebAPIJobs = 'http://117.120.7.120/spwebapi/api/jobinfoex?AssetResellerID=' + getSessionstorageValueUserResellerID + '&AssetCompanyID=' + getSessionstorageValueCompanyID;
;
        // if (getSessionstorageValueRoleID == 1) {

        //     WebAPIJobs = 'http://117.120.7.120/spwebapi/api/jobinfo?AssetResellerID=' + getSessionstorageValueUserResellerID + '&AssetCompanyID=' + getSessionstorageValueCompanyID + '&Timestamp=' + timestamp + '&RxTime=' + rxtime;
        // }
        // else if (getSessionstorageValueRoleID == 2) {
        ...
        //     WebAPIJobs = 'http://117.120.7.120/spwebapi/api/jobinfo?AssetResellerID=' + getSessionstorageValueUserResellerID + '&AssetCompanyID=' + getSessionstorageValueCompanyID + '&Timestamp=' + timestamp + '&RxTime=' + rxtime + '&Agent=' + getSessionstorageValueUser;
        // }

       	console.log('Call Center : ' + WebAPIJobs);
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    outputDiv.innerHTML += "<table id='jobsCreated' border='1'>"
                                        ...
                                        + "<th style='padding: 5px;'>" + titleJobStatus + "</th>"
                    ...
                    for (k = 0; k < res.length; k++) {
                        ...
                        if (res[k] != undefined || res[k]) {
                                ...
                                if (res[k].FlagValue == "Completed") {
                                    completed++;
                                } else if (res[k].FlagValue == "New") {
                                    newJob++;
                                } else if (res[k].FlagValue == "Accepted") {
                                    accepted++;
                                } else if (res[k].FlagValue == "Edited") {
                                    edited++;
                                } else if (res[k].FlagValue == "Cancelled") {
                                    cancelled++;
                                } 
                            ...
                            outputDiv.innerHTML += "<table id='jobsCreated' border='1'>"
                                        ...
                                        + "</td>"
                                        + "<td style='padding: 5px;'>"
                                        + "<strong><a href='javascript:google.maps.event.trigger(editjob(" + res[k].AssetID + "," + res[k].JobID + "," + res[k].Flag + "),\"click\");' style='color:#458FD2;'>" + "Edit" + '</a></strong>'
                                        + "&nbsp;&nbsp;&nbsp;&nbsp;<strong><a href='javascript:google.maps.event.trigger(deletejob(" + res[k].AssetID + "," + res[k].JobID + "," + res[k].Flag + "),\"click\");' style='color:#458FD2;'>" + "Delete" + '</a></strong>'
                                        + "</td>"
                                        + "</tr>"
                                        + "</tbody>";
                                        + "</table>";
                        ...
                        document.getElementById('countNewJob').innerHTML = newJob;
                        document.getElementById('countAcceptedJob').innerHTML = accepted;
                        document.getElementById('countEditedJob').innerHTML = edited;
                        document.getElementById('countCancelledJob').innerHTML = cancelled;
                        document.getElementById('countCompletedJob').innerHTML = completed;
...
function setZones(map, zones, callback) {
    ...
    if (getSessionstorageValueRoleID == 1) {
        ...
        url = 'http://117.120.7.120/spwebapi/api/zoneinfo?ResellerID=' + '1' + '&CompanyID=' + '1';

    } else if (getSessionstorageValueRoleID == 2) {
        url = 'http://117.120.7.120/spwebapi/api/zoneinfo?ResellerID=' + getSessionstorageValueUserResellerID + '&CompanyID=' + getSessionstorageValueCompanyID;
...
function showArrayCircle(event) {
    ...
    if (getSessionstorageValueRoleID == 1) {
        ...
        WebAPIAsset = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + '&ResellerID=' + '1' + '&CompanyID=' + '1';
        ...
        WebAPIAsset = 'http://117.120.7.120/spwebapi/api/assetinfo?UserID=' + getSessionstorageValueUserID + '&ResellerID=' + getSessionstorageValueUserResellerID + '&CompanyID=' + getSessionstorageValueCompanyID;
    ...
    if (getSessionstorageValueLanguage == "en") {
        ...
        $('#jobDestination').attr('placeholder', 'Address');
        ...
        $('#jobDestination').attr('placeholder', 'ปลายทาง');
        ...
        $('#jobDestination').attr('placeholder', 'Địa Chỉ');
        ...
        $('#jobDestination').attr('placeholder', 'Tempat Tujuan');
        $('#unit').attr('placeholder', 'Unit Nos.');
    ...
    $(function () {
        ...
        $('#companyName').autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'http://track-asia.com/spwebapi/api/autocompletecompany?CompanyID=' + getSessionstorageValueCompanyID,
                    type: 'GET',
                    cache: false,
                    data: {
        ...
        $('#jobDestination').autocomplete({
            source: function (request, response) {
                $.ajax({
                    url: 'http://track-asia.com/spwebapi/api/autocompleteaddress?CompanyID=' + getSessionstorageValueCompanyID,
                    type: 'GET',
                    cache: false,
                    data: {
